...
export class HRNChat extends EventTarget {
    constructor(customConfig = {}) {
        ...
        this.AVATARS = ['./assets/avatars/1.webp', './assets/avatars/2.webp', './assets/avatars/3.webp', './assets/avatars/4.webp', './assets/avatars/5.webp'];
    ...
    async _fullDisconnect() {
        await this._cleanupChannels(false);
    }

    async _trackPresence(channel, userId) {
    ...
    async _setupGlobalPresence(userId) {
        ...
        this.state.globalPresenceChannel.on('presence', { event: 'sync' }, async () => {
            ...
            if (this.state.user && !this.state.isOfflineMode && !this.state.isCapacityBlocked) {
                if (this.state.globalOnlineCount > this.CONFIG.maxUsers) {
                    if (!uniqueUserIds.has(this.state.user.id)) this._handleServerFull();
                }
            }
        }).subscribe(async (status) => {
            if (status === 'SUBSCRIBED' && userId) await this._trackPresence(this.state.globalPresenceChannel, userId);
        });
    }
    ...
    _attemptHardReconnect() {
        ...
        this.state.connectionTimeoutTimer = setTimeout(() => {
            this.state.isHardReconnecting = false;
            this._attemptHardReconnect();
        }, timeout);
    ...
    async _setupChatChannel(id) {
        ...
        const isDirect = this.state.currentRoomData?.is_direct;
    ...
    _startInternetCheck() {
        ...
        this.state.internetCheckInterval = setInterval(async () => {
            try {
                ...
                if (response.ok) { this._stopInternetCheck(); if (this.state.isOfflineMode) this.goOnline(); }
            } catch (e) {}
        }, 5000);
    }
    ...
    _monitorConnection() {
        const onlineHandler = () => { 
            this._stopInternetCheck(); 
            if (this.state.isOfflineMode) this.goOnline(); 
            else { 
                this._dispatch('connection_status', { status: 'connecting' }); 
                try { if (this.db.realtime && typeof this.db.realtime.connect === 'function') this.db.realtime.connect(); } catch (e) {}
                if (this.state.currentRoomId) this._attemptHardReconnect(); 
                else this._dispatch('connection_status', { status: 'connected' }); 
            } 
        };
        const offlineHandler = async () => {
            ...
            this.state.globalPresenceReady = false;
            this._cleanupChannels(false); 
        ...
        document.addEventListener('visibilitychange', async () => {
            ...
            if (document.visibilityState === 'hidden') {
                ...
                this.state.backgroundDisconnectTimer = setTimeout(async () => {
                    if (!this.state.isOfflineMode && !this.state.isCapacityBlocked) { await this._fullDisconnect(); this.state.isBackgroundDisconnectActive = true; }
                }, this.CONFIG.backgroundDisconnectMs);
                ...
                if (this.state.isBackgroundDisconnectActive) {
                    this.state.isBackgroundDisconnectActive = false;
                    if (this.state.user && !this.state.isOfflineMode) {
                        try { if (this.db.realtime && typeof this.db.realtime.connect === 'function') this.db.realtime.connect(); } catch (e) {}
                        this._setupGlobalPresence(this.state.user.id);
                        if (this.state.currentRoomId) this._attemptHardReconnect();
                    }
    ...
    async _resolveRoomDisplay(room) {
        ...
        if (!room.is_direct) return { name: room.name, avatar: room.avatar_url || null };
        const myId = this.state.user?.id;
        ...
        return { name: profile.full_name || 'User', avatar: profile.avatar_url || null };
    }
    ...
    async _cacheAvatar(profile) {
        ...
        if (this.state.profileCacheKeys.length > this.MAX_CACHE_SIZE) {
            const oldKey = this.state.profileCacheKeys.shift();
            delete this.state.profileCache[oldKey];
        }
        return profile;
    }
    ...
    async init() {
        ...
        if (navigator.onLine) {
            ...
            this._setupGlobalPresence(null);
            if (storedEmail && storedPass) {
                ...
                if (success) {
                    this._setupGlobalPresence(this.state.user.id);
                    this._dispatch('auth_state_changed', { user: this.state.user, state: 'logged_in' });
                    this.loadRooms();
                } else {
                    ...
                    if (knownUser && knownUser.pass_hash === hashInput) {
                        this.state.user = { id: knownUser.userId, email: knownUser.email, user_metadata: knownUser.metadata };
                        this.setAppMode(true);
            ...
            if (storedEmail && storedPass) {
                ...
                if (knownUser && knownUser.pass_hash === hashInput) {
                    ...
                    this._dispatch('auth_state_changed', { user: null, state: 'logged_out' });
                    // Pass credentials back if needed for UI pre-fill
                    this._dispatch('offline_credentials', { email: storedEmail, pass: storedPass });
                }
            } else {
                this._dispatch('auth_state_changed', { user: null, state: 'logged_out' });
            }
    ...
    setAppMode(offline) {
        this.state.isOfflineMode = offline;
        this._dispatch('mode_changed', { offline });
        if(offline) this._dispatch('connection_status', { status: 'offline' });
    }

    async goOnline() {
        if (this.state.isConnecting || this.state.isHardReconnecting) return;
        this.state.isConnecting = true;
        this._stopInternetCheck(); this.state.isCapacityBlocked = false;
        this._dispatch('server_full', { full: false }); // Reset block
        ...
        if (this.state.user) {
            ...
            if (storedEmail && storedPass) {
                ...
                if (success) { 
                    ...
                    if (this.state.currentRoomId) this._attemptHardReconnect(); 
                    this.loadRooms(); 
                    this._dispatch('notification', { message: "Connected." });
                }
        ...
        // If not logged in yet but trying to go online
        const storedEmail = localStorage.getItem('hrn_auth_email'); const storedPass = localStorage.getItem('hrn_auth_pass');
        if (storedEmail && storedPass) {
    ...
    stayOffline() {
        this._dispatch('server_full', { full: false });
        this._fullDisconnect();
        this.setAppMode(true);
        this._dispatch('notification', { message: "Offline mode active." });
        if (this.state.user) this.loadRooms();
    }

    async _attemptLogin(email, pass) {
        try {
            const { error } = await this._execQuery(this.db.auth.signInWithPassword({ email, password: pass }));
            if (error) throw error;
        ...
        if (!navigator.onLine) {
            ...
            if (knownUser && knownUser.metadata) { 
                ...
                if (knownUser.pass_hash && knownUser.pass_hash === hashInput) { 
                    ...
                    this._dispatch('notification', { message: "Offline login successful." }); 
                    return; 
                } 
            }
            this._dispatch('notification', { message: "No internet and no offline account found." });
            this._dispatch('loading', { active: false }); this.state.processingAction = false; return;
        }

        const success = await this._attemptLogin(email, pass);
        if (success) { 
            localStorage.setItem('hrn_auth_email', email); localStorage.setItem('hrn_auth_pass', pass); 
            this.setAppMode(false); 
            if (this.state.user) this._setupGlobalPresence(this.state.user.id); 
            this._dispatch('auth_state_changed', { user: this.state.user, state: 'logged_in' });
            this.loadRooms(); 
            this._dispatch('notification', { message: "Connected." });
        }
    ...
    async register(stepData) {
        // step 1: just validation usually, step 2: send code
        if (this.state.processingAction) return;
        this.state.processingAction = true;
        ...
        try {
            ...
            if (r) {
                ...
                if (j.message === "Code sent") { 
                    sessionStorage.setItem('temp_reg', JSON.stringify({ name, email, pass, avatar: avatarUrl })); 
                    this._dispatch('registration_code_sent', { email }); 
                }
    ...
    async verifyCode(code) {
        ...
        try {
            ...
            if (r.status === 429) { this._dispatch('notification', { message: "Too many attempts." }); this.state.processingAction = false; this._dispatch('loading', { active: false }); return; }
            const j = await r.json();
            if (j.message === "Verified") await this._finishReg(temp); 
            else { this._dispatch('notification', { message: "Invalid code." }); }
    ...
    async _finishReg(temp) {
        ...
        if (error) { this._dispatch('notification', { message: "Registration failed." }); }
            ...
            localStorage.setItem('hrn_auth_email', temp.email); localStorage.setItem('hrn_auth_pass', temp.pass); 
            // Auto login logic would trigger here normally via auth listener, but we can force it
            this._dispatch('registration_success');
        }
    }

    async logout() {
        this._dispatch('loading', { active: true, text: "Leaving..." });
        this.state.currentRoomId = null; this.state.user = null; 
        await this._fullDisconnect();
        localStorage.removeItem('hrn_auth_email'); localStorage.removeItem('hrn_auth_pass');
        this.setAppMode(false); this.state.isCapacityBlocked = false; 
        ...
        if (!this.state.isOfflineMode) {
            try {
                ...
                if (serverProfile) {
                    ...
                    if (!profile || serverTime > localTime || !profile.full_name) { profile = serverProfile; await this._cacheAvatar(profile); }
                }
    ...
    async loadRooms() {
        ...
        const processAndRender = async (rooms) => { 
            ...
            for (const r of rooms) { 
                if (!r || !r.id) continue; 
                const display = await this._resolveRoomDisplay(r); 
                processed.push({ ...r, display_name: display.name, display_avatar: display.avatar }); 
            } 
            this.state.allRooms = processed; 
            this._dispatch('rooms_updated', { rooms: processed }); 
        };
        
        if (this.state.isOfflineMode) { 
            const localRooms = await this.localDB.getAll('rooms'); 
            await processAndRender(localRooms); 
            return; 
        }
        this._dispatch('loading', { active: true, text: "Syncing..." });
        try { 
            const { data: rooms, error } = await this._execQuery(this.db.from('rooms').select('*').order('created_at', { ascending: false })); 
            if (error) throw error; 
            if (rooms) { 
                await this.localDB.clear('rooms'); 
                if (rooms.length > 0) await this.localDB.putAll('rooms', rooms); 
                await this.localDB.saveUserTree(uid, rooms); 
                await processAndRender(rooms); 
            } 
        } catch (e) { this._dispatch('notification', { message: "Sync failed." }); }
        this._dispatch('loading', { active: false });
    }
    ...
    async createRoom(details) {
        ...
        if (type === 'direct') {
            ...
            await this.localDB.put('profiles', profile); this.state.profileCache[profile.id] = profile;
             ...
             if (!name) { this._dispatch('notification', { message: "Name required." }); this.state.processingAction = false; this._dispatch('loading', { active: false }); return; }
        }

        this._dispatch('loading', { active: true, text: "Creating..." });
        const roomSalt = this._generateSalt();
        let finalAllowed = allowedUsers || [];
        if (type === 'direct') finalAllowed = [this.state.user.id, targetUser];
        else {
            if (finalAllowed.length === 0) finalAllowed = ['*'];
            if (!finalAllowed.includes(this.state.user.id)) finalAllowed.push(this.state.user.id);
        }
        ...
        const insertData = { 
            ...
            is_visible: type === 'direct' ? true : visible, 
            salt: roomSalt, 
        ...
        if (data && data.length > 0) {
            ...
            if (password) { 
                const accessHash = await this._sha256(password + roomSalt); 
                await this.db.rpc('set_room_password', { p_room_id: newRoom.id, p_hash: accessHash }); 
            }
            await this.localDB.put('rooms', newRoom); 
            this._dispatch('room_created', { room: newRoom, password });
        }
    ...
    async joinRoom(id, password = null) {
        const openLocal = async () => { 
            const meta = await this.localDB.get('rooms', id); 
            if (meta && meta.id) { 
                if (meta.has_password && !password) {
                    this.state.pendingRoomEntry = meta;
                    this._dispatch('room_password_required', { room: meta });
                    return;
                }
                await this._openVault(meta.id, meta.name, password, meta.salt, meta); 
            } else this._dispatch('notification', { message: "Chat not found locally." }); 
        };
        ...
        try {
            ...
            if (data.has_password && !password) {
                this.state.pendingRoomEntry = data;
                this._dispatch('room_password_required', { room: data });
            } else {
                await this._openVault(data.id, data.name, password, data.salt, data);
            }
        } catch (e) { 
            this._dispatch('loading', { active: false }); 
            this._dispatch('notification', { message: "Connection lost." }); 
            this.setAppMode(true); 
            await openLocal(); 
        }
    }
    ...
    async submitGatePassword(password) {
        ...
        if (this.state.isOfflineMode) { 
            await this._openVault(this.state.pendingRoomEntry.id, this.state.pendingRoomEntry.name, password, this.state.pendingRoomEntry.salt, this.state.pendingRoomEntry); 
            return; 
        }
        const inputHash = await this._sha256(password + this.state.pendingRoomEntry.salt);
        this._dispatch('loading', { active: true, text: "Verifying..." });
        const { data } = await this.db.rpc('verify_room_password', { p_room_id: this.state.pendingRoomEntry.id, p_hash: inputHash }); 
        this._dispatch('loading', { active: false });
        if (data === true) {
            await this._openVault(this.state.pendingRoomEntry.id, this.state.pendingRoomEntry.name, password, this.state.pendingRoomEntry.salt, this.state.pendingRoomEntry);
            this.state.pendingRoomEntry = null;
    ...
    async _openVault(id, n, rawPassword, roomSalt, roomData) {
        ...
        await this._deriveKey(keySource, roomData?.salt, id); 
        
        let finalMessages = [];
        
        if (this.state.isOfflineMode) {
            let localMessages = await this.localDB.getRoomMessages(id);
            finalMessages = localMessages;
            this._dispatch('room_entered', { room: roomData, display, messages: finalMessages, isOffline: true });
            this._dispatch('loading', { active: false });
            return;
        }
        ...
        try {
            ...
            if (data && data.length > 0) {
                data.reverse(); const res = await this._workerExec('decryptHistory', { messages: data, keyId: id }); const validMsgs = res.results.filter(m => !m.error);
                await this.localDB.clearRoomMessages(id); const messagesWithRoomId = validMsgs.map(m => ({ ...m, room_id: id })); await this.localDB.putAll('messages', messagesWithRoomId); finalMessages = validMsgs;
        ...
        this._dispatch('connection_status', { status: 'connecting' });
        await this._initRoomPresence(id); 
        await this._setupChatChannel(id);
    ...
    async sendMessage(text) {
        ...
        try { 
            const enc = await this._encryptMessage(text, this.state.currentRoomId); 
            const { error } = await this.db.from('messages').insert([{ room_id: this.state.currentRoomId, user_id: this.state.user.id, user_name: this.state.user.user_metadata?.full_name, content: enc }]).select().single(); 
            if (error) this._dispatch('notification', { message: "Failed to send." }); 
        } catch (err) { this._dispatch('notification', { message: "Send failed." }); }
        this.state.processingAction = false;
    }

    async editMessage(id, newText) {
        if (!this.state.user || !id || !newText) return;
        // 15 min check logic is UI dependent usually, but let's assume UI checked or we check here.
        this._dispatch('loading', { active: true, text: "Saving..." });
        try { 
            const enc = await this._encryptMessage(newText, this.state.currentRoomId); 
            const { error } = await this.db.from('messages').update({ content: enc }).eq('id', id); 
            if (error) this._dispatch('notification', { message: "Failed to edit." }); 
            else this._dispatch('notification', { message: "Message updated." }); 
        } catch (e) { this._dispatch('notification', { message: "Encryption failed." }); }
        this._dispatch('loading', { active: false });
    }
    ...
    async loadMoreHistory() {
        ...
        this._dispatch('history_loading', { loading: true });
        ...
        if (error || !data || data.length === 0) { 
            ...
            this.state.isLoadingHistory = false; 
            this._dispatch('history_loaded', { messages: [], hasMore: false });
            return; 
        }
        ...
        try {
            ...
            if (validMsgs.length > 0) {
                ...
                await this.localDB.putAll('messages', validMsgs.map(m => ({ ...m, room_id: this.state.currentRoomId })));
                this._dispatch('history_loaded', { messages: validMsgs, hasMore: true });
            }
        } catch (e) {}
        this.state.isLoadingHistory = false;
    }
    ...
    async updateRoomSettings(settings) {
        if (this.state.processingAction) return;
        this.state.processingAction = true;
        const { name, isVisible, newPassword, removePassword, allowedUsers } = settings;
        ...
        const { error: updateError } = await this.db.from('rooms').update(updates).eq('id', this.state.currentRoomId);
        ...
        if (isRemovingPass) { 
            await this.db.rpc('set_room_password', { p_room_id: this.state.currentRoomId, p_hash: null }); 
            this.state.currentRoomData.has_password = false; 
        }
        else if (isChangingPass) { 
            const roomSalt = this.state.currentRoomData.salt; 
            const accessHash = await this._sha256(newPassword + roomSalt); 
            await this.db.rpc('set_room_password', { p_room_id: this.state.currentRoomId, p_hash: accessHash }); 
            this.state.currentRoomData.has_password = true; 
        }
        
        const { data: updatedRoom } = await this.db.from('rooms').select('*').eq('id', this.state.currentRoomId).single();
    ...
    async deleteRoom(id) { 
        if (!id) return; 
        this._dispatch('loading', { active: true, text: "Deleting..." }); 
        const { error } = await this.db.from('rooms').delete().eq('id', id); 
        ...
        this.state.currentRoomId = null; this.state.currentRoomData = null; 
        this._dispatch('room_left'); // Go back to lobby
        this.loadRooms(); 
        this._dispatch('loading', { active: false }); 
    };
}
